...
if (($homepath = getenv("MEDKONNECTEHRPATH")) === false) {
  if (!is_file("MEDKONNECTEHRPATH") or ($homepath = file_get_contents("MEDKONNECTEHRPATH")) === false) {
    if ($iscli) {
      die("La variable d'environnement MEDKONNECTEHRPATH n'a pas été fixée.\nFaites un 'export MEDKONNECTEHRPATH=/chemin/vers/MedKonnectEHR\nAlternativement, vous pouvez créer un fichier 'MEDKONNECTEHRPATH' contenant '/chemin/vers/MedKonnectEHR' et le placer dans le dossier web de MedKonnectEHR.\n");
    } else {
...
if (!is_dir($homepath . "vendor")) {
  die("L'installation de MedKonnectEHR ne semble pas complète, veuillez installer COMPOSER (<a href='https://getcomposer.org'>https://getcomposer.org</a>)\nTapez ensuite <code>composer update</code> en ligne de commande dans le répertoire d'installation de MedKonnectEHR.\n");
}
if (!is_dir($webdir . "thirdparty")) {
  die("L'installation de MedKonnectEHR ne semble pas complète, veuillez lancer <code>composer.phar install</code> dans le dossier " . $webdir);
}
...
if (!is_file($homepath . 'config/config.yml')) {
  ...
  if (!$iscli && $_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (empty(msSQL::sql2tabSimple("SHOW TABLES"))) {
      ...
      exec('mysql -u ' . escapeshellarg($p['config']['sqlUser']) . ' -p' . escapeshellarg($p['config']['sqlPass']) . ' -h' . escapeshellarg($p['config']['sqlServeur']) . ' --default-character-set=utf8 ' . escapeshellarg($p['config']['sqlBase']) . ' < ' . $homepath . 'upgrade/base/sqlInstallMedTn.sql');
      ...
      foreach ($modules as $module) {
        if ($module != '.' and $module != '..') {
          ...
          $command = 'mysql -u ' . escapeshellarg($p['config']['sqlUser']) . ' -p' . escapeshellarg($p['config']['sqlPass']) . ' -h' . escapeshellarg($p['config']['sqlServeur']) . ' --default-character-set=utf8 ' . escapeshellarg($p['config']['sqlBase']) . ' < ' . $homepath . 'upgrade/' . $module . '/sqlInstall.sql';
          exec($command, $output, $status);
          echo "******** Module Command: $command\n";
          echo "******** Output: " . implode("\n", $output) . "\n";
          echo "******** Status: $status\n";
        }
      }
