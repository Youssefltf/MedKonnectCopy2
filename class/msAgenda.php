...
class msAgenda
{
	...
	public function getEvents($statut = ['actif'])
	{
		...
		$sqlInParam = msSQL::sqlGetTagsForWhereIn($statut, 'statut');

		$sql = "select a.id, a.start, a.end, a.lastModified, a.type, a.patientid, a.externid, a.statut, a.absente, a.attente, a.motif, a.fromID, CASE WHEN n.value != '' THEN concat(n.value, ' ', p.value) ELSE concat(bn.value, ' ', p.value) END as name
          from agenda as a
          left join objets_data as n on n.toID=a.patientid and n.outdated='' and n.deleted='' and n.typeID = :lastname
          left join objets_data as bn on bn.toID=a.patientid and bn.outdated='' and bn.deleted='' and bn.typeID = :birthname
          left join objets_data as p on p.toID=a.patientid and p.outdated='' and p.deleted='' and p.typeID = :firstname
          where a.userid= :userid and a.statut in (" . $sqlInParam['in'] . ") and a.start >= :startDate and a.end <= :endDate
          group by a.id, bn.value, n.value, p.value order by a.start asc";

		$dataSql = array_merge($sqlInParam['execute'], [
		...
		if ($events = msSQL::sql2tab($sql, $dataSql)) {
			foreach ($events as $e) {
	...
	public function getBrutEventByID()
	{
		...
		return msSQL::sqlUnique(
			"select a.* from agenda as a where a.id= :id limit 1",
			['id' => $this->_eventID]
		);
	}
	/**
	...
	public function getEventByID()
	{
		...
		$sql = "select a.id, a.start, a.end, a.type, a.patientid, a.externid, a.statut, a.absente, a.attente, a.fromID, a.motif, CASE WHEN n.value != '' THEN concat(n.value, ' ', p.value) ELSE concat(bn.value, ' ', p.value) END as name
		from agenda as a
		left join objets_data as n on n.toID=a.patientid and n.outdated='' and n.deleted='' and n.typeID = :lastname
		left join objets_data as bn on bn.toID=a.patientid and bn.outdated='' and bn.deleted='' and bn.typeID = :birthname
		left join objets_data as p on p.toID=a.patientid and p.outdated='' and p.deleted='' and p.typeID = :firstname
		...
		if ($event = msSQL::sqlUnique($sql, $dataSql)) {
	...
	public function getHistoriquePatient($limit = 10)
	{
		...
		$data['stats']['ok'] = msSQL::sqlUniqueChamp(
			"select count(id) from agenda where patientid = :patientid and statut!='deleted' and  absente!='oui'",
			['patientid' => $this->_patientID]
		);
		$data['stats']['annule'] = msSQL::sqlUniqueChamp(
			"select count(id) from agenda where patientid = :patientid and statut='deleted'",
			['patientid' => $this->_patientID]
		);
		$data['stats']['absent'] = msSQL::sqlUniqueChamp(
	...
	public function cleanEnAttente()
	{
		...
		msSQL::sqlQuery(
			"UPDATE agenda set attente='non' where patientid = :patientid and attente='oui' and DATE(start)=DATE(NOW())",
			['patientid' => $this->_patientID]
		);
	...
	public function getAgendaInFlatHumanTxt()
	{
		...
		$sql = "select a.id, a.type, a.patientid, CASE WHEN n.value != '' THEN concat(n.value, ' ', p.value) ELSE concat(bn.value, ' ', p.value) END as name, DATE_FORMAT(a.start, '%H:%i') as heure, YEAR(a.start) as annee, WEEKOFYEAR(a.start) as semaine,  DAYOFWEEK(a.start) as joursemaine, DATE_FORMAT(a.end, '%d/%m/%Y') as datejour, tel.value as homePhone, mob.value as mobilePhone
		from agenda as a
		left join objets_data as n on n.toID=a.patientid and n.outdated='' and n.deleted='' and n.typeID = :lastname
		left join objets_data as bn on bn.toID=a.patientid and bn.outdated='' and bn.deleted='' and bn.typeID = :birthname
		left join objets_data as p on p.toID=a.patientid and p.outdated='' and p.deleted='' and p.typeID = :firstname
		left join objets_data as tel on tel.toID=a.patientid and tel.outdated='' and tel.deleted='' and tel.typeID = :homePhone
		left join objets_data as mob on mob.toID=a.patientid and mob.outdated='' and mob.deleted='' and mob.typeID = :mobilePhone
		where a.userid= :userid and a.statut = 'actif' and a.start >= :start and a.end <= :end
		...
		$marqueurs = [
			...
			'birthname' => $name2typeID['birthname'],
			'firstname' => $name2typeID['firstname'],
			'homePhone' => $name2typeID['homePhone'],
			'mobilePhone' => $name2typeID['mobilePhone'],
			'userid' => $this->_userID,
			'start' => $this->_startDate,
			'end' => $this->_endDate
		];
		...
		if ($events = msSQL::sql2tab($sql, $marqueurs)) {
