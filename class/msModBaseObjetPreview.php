...
class msModBaseObjetPreview
{
	...
	public function getGenericPreviewReglement()
	{
    	...
    	LEFT JOIN objets_data as od ON dt.id=od.typeID WHERE od.id = :objetID limit 1", ['objetID' => $this->_objetID]);
		$p['page']['acteFacture'] = msSQL::sqlUnique("SELECT * FROM actes WHERE id=(SELECT parentTypeID FROM objets_data WHERE id = :objetID )", ['objetID' => $this->_objetID]);
	...
	public function getGenericPreviewOrdo()
	{
		...
		$marqueurs['objetID'] = $this->_objetID;

		if ($ordoData = msSQL::sql2tab("SELECT ald.value as ald, p.value as description, p.typeID, p.id
		from objets_data as p
		left join objets_data as ald on p.id=ald.instance and ald.typeID = :ordoLigneOrdoALDouPas and ald.outdated='' and ald.deleted=''
		where p.instance = :objetID and p.outdated='' and p.deleted='' and p.typeID in ( :ordoTypeImpression , :ordoLigneOrdo )
		group by p.id, ald.id
		order by p.id asc", $marqueurs)) {
			$modePrint = 'standard';
