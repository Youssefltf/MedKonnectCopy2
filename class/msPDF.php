...
/**
 * Générer du PDF
 * MedKonnectEHR utilise des templates Twig <https://twig.sensiolabs.org/>
...
class msPDF
{
	...
	private function _makeBodyForOrdo()
	{
		...
		if ($ordoData = msSQL::sql2tab("SELECT p.*, ald.value as ald
          ...
          left join objets_data as ald on p.id=ald.instance and ald.typeID= :typeID and ald.outdated='' and ald.deleted=''
          where p.instance= :instance and p.outdated='' and p.deleted=''
          group by p.id, ald.value
          order by p.id asc", ['typeID' => $name2typeID['ordoLigneOrdoALDouPas'], 'instance' => $this->_objetID])) {
	...
	public function makeWithTwig($template)
	{
		...
		if (isset($p['config']['twigEnvironnementCache'])) $twigEnvironment['cache'] = $p['config']['twigEnvironnementCache'];
		else $twigEnvironment['cache'] = false;
		if (isset($p['config']['twigEnvironnementAutoescape'])) $twigEnvironment['autoescape'] = $p['config']['twigEnvironnementAutoescape'];
		else $twigEnvironment['autoescape'] = false;
	...
	private function _savePdfFromTemplatePdf()
	{
		...
		if (isset($pdfDat['defautFontSize'])) $defautFontSize = $pdfDat['defautFontSize'];
		else $defautFontSize = 10;
		if (isset($pdfDat['defautTextColor'])) $defautTextColor = explode(',', $pdfDat['defautTextColor']);
		else $defautTextColor = [0, 0, 0];
		if (isset($pdfDat['defautFont'])) $defautFont = $pdfDat['defautFont'];
		else $defautFont = 'Arial';
