...
class msMotSuivi
{
	...
	public function update()
	{
		...
		$res = msSQL::sqlInsert(self::TABLE, $data);
	...
	public function delete()
	{
		...
		$res = msSQL::sqlQuery($sql, ['id' => $this->_id]);
	...
	public function setTexte(string $texte)
	{
		...
		$this->_texte = substr(trim($texte), 0, 255);
		return true;
	}
	...
	public static function getList(int $toID, int $nb_elem = 0)
	{
		...
		$sql .= ' LEFT JOIN `objets_data` AS ln ON ln.toID = ms.fromID AND ln.typeID = :typeIDlastname AND ln.outdated = \'\' AND ln.deleted = \'\'';
		$sql .= ' LEFT JOIN `objets_data` AS bn ON bn.toID = ms.fromID AND bn.typeID = :typeIDbirthname AND bn.outdated = \'\' AND bn.deleted = \'\'';
		$sql .= ' LEFT JOIN `objets_data` AS fn ON fn.toID = ms.fromID AND fn.typeID = :typeIDfirstname AND fn.outdated = \'\' AND fn.deleted = \'\'';
		$sql .= ' WHERE ms.toID = :toID ORDER BY ms.dateTime DESC, id DESC';
		if (is_int($nb_elem) and $nb_elem >= 0) $sql .= ' LIMIT ' . $nb_elem;

		$res = msSQL::sql2tab(
			$sql,
			[
				'typeIDlastname' => $dataTypeIDs['lastname'],
				'typeIDbirthname' => $dataTypeIDs['birthname'],
				'typeIDfirstname' => $dataTypeIDs['firstname'],
				'toID' => $toID
			]
		);
		return $res;
	...
	public static function getNbTotal(int $toID)
	{
		...
		$total = (int) msSQL::sqlUniqueChamp(
			$sql,
			['toID' => $toID]
		);
		return $total;
