...
class msExportData
{
	...
	public function getExportabledList($namePrefix = [])
	{
		...
		if (!empty($namePrefix)) {
			...
			foreach ($namePrefix as $prefix) {
				$wherePrefix[] = ' name like :marq' . $i . ' ';
				$marqueurs['marq' . $i] = $prefix . '%';
				$i++;
			}
		...
		if ($data =  msSQL::sql2tab("SELECT id, label, cat, formValues from data_types where groupe='typecs' and formType='select' " . $wherePrefix . " order by displayOrder", $marqueurs)) {
			foreach ($data as $v) {
				$tab[$v['cat']][] = $v;
			}
			return $tab;
	...
	private function _getAllObjetsID()
	{
		...
		if (!empty($p['config']['statsExclusionPatients'])) {
			$sqlImplodePa = msSQL::sqlGetTagsForWhereIn(explode(',', $p['config']['statsExclusionPatients']), 'patientExc');
			$toIdToExclude = " and toID not in (" . $sqlImplodePa['in'] . ") ";
			$marqueurs = array_merge($marqueurs, $sqlImplodePa['execute']);
		} else {
			$toIdToExclude = '';
		}
		if ($this->_canExportAll == true) {
			...
			$sqlImplodePra = msSQL::sqlGetTagsForWhereIn($this->_pratList, 'prat');
			$fromIdwhere = "and fromID in (" . $sqlImplodePra['in'] . ")";
			$marqueurs = array_merge($marqueurs, $sqlImplodePra['execute']);
		}
		...
		if ($p['config']['optionGeActiverRegistres'] == 'true' and $p['config']['optionGeExportDataConsentementOff'] != 'true' and isset($this->_registreID)) {
			if (!empty($this->_getAllPatientsIncludedInRegistry())) {
				$sqlImplodePaInc = msSQL::sqlGetTagsForWhereIn($this->_patientsIncludedInRegistry, 'patientInc');
				$toIdwhere = " and toID in (" . $sqlImplodePaInc['in'] . ") ";
				$marqueurs = array_merge($marqueurs, $sqlImplodePaInc['execute']);
			}
		}

		$sqlImplodeDataTy = msSQL::sqlGetTagsForWhereIn($this->_dataTypeIDs, 'dataTypeID');
		$marqueurs = array_merge($marqueurs, $sqlImplodeDataTy['execute']);

		return $this->_allObjetsID = msSQL::sql2tabSimple("SELECT id from objets_data where typeID in (" . $sqlImplodeDataTy['in'] . ") " . $fromIdwhere . $toIdwhere . " " . $toIdToExclude . " " . $this->_formatDateParameters() . " and outdated='' and deleted=''", $marqueurs);
	}
	...
	private function _getAllPratAdminData()
	{
		...
		$sqlImplode = msSQL::sqlGetTagsForWhereIn($this->_allObjetsID, 'obID');

		if ($this->_allPratID = msSQL::sql2tabSimple("SELECT distinct(fromID) from objets_data where id in (" . $sqlImplode['in'] . ")", $sqlImplode['execute'])) {
	...
	private function _getAllObjetsAndChildsData()
	{
		...
		$sqlImplodeFF = msSQl::sqlGetTagsForWhereIn($this->getFormFieldList(), 'obID');
		$marqueurs = array_merge($marqueurs, $sqlImplodeFF['execute']);
		...
		where (o.id in (" . $sqlImplode['in'] . ") or (o.instance in (" . $sqlImplode['in'] . ") and t.name in (" . $sqlImplodeFF['in'] . "))) and o.outdated='' and o.deleted='' ", 'id', '', $marqueurs);
		if ($data) {
			foreach ($data as $k => $v) {
