...
class msBDPM
{
	...
	public function get_the_spe_txt($libprod, $monovir)
	{
		...
		if ($monovir == '0') {
			...
			'' as sp_cipucd13
			from bdpm_specialitesVirtuelles
			where monovir = '1' and denomination like :libprod
			order by denomination
			limit 150
		", ['libprod' => $libprod]);
		} elseif ($monovir == '3') {
	...
	public function get_the_specialite_multi_codeid($codeid, $vartyp, $monovir)
	{
		...
		if ($vartyp === 1) {
			...
			from bdpm_specialitesVirtuelles as s
			left join bdpm_groupesGeneriques as g on g.codeCIS = s.codeCIS
			where s.codeSPE in ( :codeid ) " . $monovirSearch . "
		", ['codeid' => $codeid]);
		} elseif ($vartyp == 7) {
			return  msSQL::sql2tab("SELECT
			...
			where c.codeSubstance in ( :codeid )
		", ['codeid' => $codeid]);
		}
	}
	...
	public function get_the_sub_txt($libtxt, $vartype)
	{
		...
		from bdpm_compositions as c
		where c.denomination like :libtxt
		", ['libtxt' => $libtxt]);
	}
	...
	public function get_the_sub_spe($codeid, $typeid)
	{
		if ($typeid === 2) {
			...
			from bdpm_compositions as c
			where nature = 'SA' and codeCIS = :codeid
		", ['codeid' => $codeid]);
		}
	}
	...
	public function get_the_presdel($codeid, $typid)
	{
		if ($typid < 1) {
			...
			if ($data = msSQL::sql2tab("SELECT `condition` from bdpm_conditions where codeCIS = :codeid", ['codeid' => $codeid])) {
				foreach ($data as $v) {
	...
	public function get_the_unite($codeid, $typid)
	{
		$data = [];
		if ($typid == 0) {
			$codeid = msSQL::sqlUniqueChamp("SELECT codeCIS from bdpm_specialitesVirtuelles where codeSPE = :codeid limit 1", ['codeid' => $codeid]);
		}
	...
	public function get_the_denomination_commune($typid, $var, $dc)
	{
		if ($typid == 2) {
			if ($data = msSQL::sql2tab("SELECT
				...
				where g.codeCIS = :var and typeGene = 0
				limit 1", ['var' => $var])) {
				return $data;
	...
	public function get_the_prix_unit_est($list_code, $typid)
	{
		if ($typid === 2) {
			...
			$sqlImplode = msSQL::sqlGetTagsForWhereIn(explode(',', $list_code), 'co');

			return msSQL::sql2tab("SELECT
			p.codeCIS as code,
			REPLACE(p.prix1, ',', '.' ) as prix
			from bdpm_presentationsVirtuelles as p
			where p.prix1 != '' and p.codeCIS in (" . $sqlImplode['in'] . ");
		", $sqlImplode['execute']);
		}
	}
}
