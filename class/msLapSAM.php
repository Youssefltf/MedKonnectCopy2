...
class msLapSAM
{
	...
	public function getSamPorteurData()
	{
		...
		return msSQL::sqlUnique("SELECT pd.*
		from objets_data as pd
		where pd.typeID = :lapSam and pd.value = :samID and pd.deleted='' and pd.outdated=''
		order by updateDate desc
		limit 1", ['samID' => $this->_samID, 'lapSam' => $name2typeID['lapSam']]);
	}
	...
	{
		$obj = new msObjet();
		$obj->setToID(msPeopleSearch::getServiceID('medkonnect'));
		$obj->setFromID(msPeopleSearch::getServiceID('medkonnect'));
	...
	{
		if ($this->getSamPorteurData()) {
			$name2typeID = new msData;
			$name2typeID = $name2typeID->getTypeIDsFromName(['lapSamDisabled']);

			$marqueurs = [
				'lapSamDisabled' => $name2typeID['lapSamDisabled'],
				'toID' => $this->_toID,
				'fromID' => $this->_fromID
			];

			if ($msSQL::sqlUnique("SELECT pd.id
			from objets_data as pd
			where pd.typeID = :lapSamDisabled and pd.toID = :toID and pd.fromID = :fromID and pd.deleted='' and pd.outdated=''
			order by updateDate desc
			limit 1", $marqueurs)) {
				return 'disabled';
			} else {
	...
	public function setSamDisabledForPatient()
	{
		...
		$obj->createNewObjetByTypeName('lapSamDisabled', '', $porteurID);
	}
	...
	public function setSamEnabledForPatient()
	{
		if ($porteur = $this->getSamPorteurData()) {
			...
			from objets_data as pd
			where pd.typeID = :lapSamDisabled and pd.toID = :toID and pd.fromID = :fromID and pd.instance = :porteurID and pd.deleted='' and pd.outdated=''
			order by updateDate desc
			limit 1", $marqueurs)) {
	...
	public function getDisabledPatientsListForSam()
	{
		if ($porteur = $this->getSamPorteurData()) {
			$name2typeID = new msData;
			$marqueurs = $name2typeID->getTypeIDsFromName(['lapSamDisabled', 'lastname', 'birthname', 'firstname']);
			$marqueurs['porteurID'] = $porteur['id'];
			$marqueurs['fromID'] = $this->_fromID;
			...
			left join objets_data as n on n.toID=o.toID and n.typeID = :lastname and n.outdated='' and n.deleted=''
			left join objets_data as bn on bn.toID=o.toID and bn.typeID = :birthname and bn.outdated='' and bn.deleted=''
			left join objets_data as p on p.toID=o.toID and p.typeID = :firstname and p.outdated='' and p.deleted=''
			where o.typeID = :lapSamDisabled and o.instance = :porteurID and o.deleted='' and o.outdated='' and o.fromID= :fromID", $marqueurs);
		} else {
			return [];
		}
	}
	...
	public function getSamCommentForPatient()
	{
		...
		$marqueurs = [
			'lapSamCommentaire' => $name2typeID['lapSamCommentaire'],
			'toID' => $this->_toID,
			'fromID' => $this->_fromID,
			'porteurID' => $porteur['id']
		];
		...
		from objets_data as pd
		where pd.typeID = :lapSamCommentaire and pd.toID = :toID and pd.fromID = :fromID and pd.instance = :porteurID and pd.deleted='' and pd.outdated=''
		order by updateDate desc
		limit 1", $marqueurs);
