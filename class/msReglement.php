...
class msReglement
{
	...
	public function setObjetID($objetID)
	{
        ...
        WHERE od.id = :objetID limit 1", ['objetID' => $objetID])) {
	...
	public function getFacturesTypesMenus()
	{
        ...
        where a.toID in ('0', :userID ) and c.module = :module and a.active='oui'
        group by a.id
        order by c.displayOrder, c.label asc, a.label asc", ['userID' => $this->_userID, 'module' => $this->_module])) {
	...
	{
		if ($factureTypeID = msSQL::sqlUniqueChamp("SELECT parentTypeID from objets_data where id = :objetID limit 1 ", ['objetID' => $this->_objetID])) {
	...
	{
		return $this->_prevalues = msSQL::sql2tabKey("SELECT typeID, value from objets_data where id= :objetID or instance = :objetID", 'typeID', 'value', ['objetID' => $this->_objetID]);
	}
	...
	public function getActeDataFromTerm($search)
	{
		...
		$marqueurs = [
			...
			'searchO' => $searcho,
			'searchPre' => '%' . $search,
			'secteurTarifaireNgap' => $this->_secteurTarifaireNgap
		];
		if ($data =  msSQL::sql2tab("SELECT *
        from actes_base
        where (code like :search or label like :searchPre ) and ((type='NGAP' and codeProf= :secteurTarifaireNgap ) or type in ('CCAM', 'Libre'))
        order by code = :searchO desc, code like search desc
        limit 25", $marqueurs)) {
			foreach ($data as $k => $v) {
				$acte = new msReglementActe;
