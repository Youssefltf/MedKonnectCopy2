...
class msStockage
{
	...
	public function getFromID()
	{
		...
		if ($d = msSQL::sqlUnique("SELECT toID, fromID from objets_data where id = :id limit 1", ['id' => $this->_objetID])) {
	...
	public function getFileExtOfDoc()
	{
		...
		if ($ext = msSQL::sqlUniqueChamp("SELECT value from objets_data where instance = :objetID and typeID = :typeID limit 1", ['objetID' => $this->_objetID, 'typeID' => $docTypeID])) {
			$this->_fileExt = $ext;
			return $ext;
		} else {
			return 'pdf';
	...
	public function getDocOrigin()
	{
		...
		$marqueurs = [
			'docOrigine' => msData::getTypeIDFromName('docOrigine'),
			'objetID' => $this->_objetID
		];
		if (msSQL::sqlUniqueChamp("SELECT value from objets_data where typeID = :docOrigine and instance = :objetID limit 1", $marqueurs) == 'interne') {
			return 'interne';
		} else {
			return 'externe';
		}
