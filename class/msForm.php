...
class msForm
{
	...
	public function setFormID($formID)
	{
		if (is_numeric($formID)) {
			if (!isset($this->_formIN)) {
				if ($formIN = msSQL::sqlUniqueChamp("SELECT internalName from forms where id= :id limit 1", ['id' => $formID])) {
					$this->_formIN = $formIN;
				} else {
	...
	public function setFormIDbyName($formName)
	{
		...
		if ($formID = msSQL::sqlUniqueChamp("SELECT id from forms where internalName= :formName limit 1", ['formName' => $formName])) {
	...
	public function getPrevaluesForPatient($patientID)
	{
		...
		$marqueurs = [];
		$marqueurs['toID'] = $patientID;

		if ($this->_instance > 0) {
			$marqueurs['instance'] = $this->_instance;
			$where = " and instance= :instance ";
		} else {
			$where = null;
		}

		$sqlImplode = msSQL::sqlGetTagsForWhereIn($this->formExtractDistinctTypes(), 'typeID');
		$marqueurs = array_merge($marqueurs, $sqlImplode['execute']);

		if ($this->_prevalues = msSQL::sql2tabKey("SELECT typeID, value from objets_data where typeID in (" . $sqlImplode['in'] . ") and toID= :toID and outdated='' and deleted='' " . $where, "typeID", "value", $marqueurs)) {
	...
	public static function getCatNameFromCatID($id)
	{
		...
		return msSQL::sqlUniqueChamp("SELECT name from forms_cat where id = :id limit 1", ['id' => $id]);
	}
	...
	{
		return msSQL::sql2tabSimple("SELECT id from data_types where groupe='typecs' and formValues= :formIN", ['formIN' => $this->_formIN]);
	}
	...
	public function getFormRawData($fields = ['*'])
	{
		...
		if ($fields != ['*']) {
			if (!msSQL::sqlValidQueryCols($fields, self::COL_TABLE_FORMS)) {
				throw new Exception('nom de colonne non valide');
			}
		}
		return msSQL::sqlUnique("SELECT " . implode(', ', $fields) . " from forms where id= :id limit 1", ['id' => $this->_formID]);
	}
	...
	{
		if (!empty($field)) {
			if (!msSQL::sqlValidQueryCols([$field], self::COL_TABLE_FORMS)) {
				throw new Exception('nom de colonne non valide');
			}
		}
		return msSQL::sqlUniqueChamp("SELECT " . $field . " from forms where internalName = :formIN limit 1", ['formIN' => $formIN]);
	}
	...
	public static function getFormUniqueRawFieldByFormID($formID, $field)
	{
		...
		return msSQL::sqlUniqueChamp("SELECT " . $field . " from forms where id = :formID limit 1", ['formID' => $formID]);
	}
	...
	public function getFormFromDb()
	{
		...
		if ($this->_formIN == 'baseLogin') {
			...
			$sql = "SELECT yamlStructure, formMethod, formAction, cda from forms where id = :id limit 1";
		}
		if ($formyaml = msSQL::sqlUnique($sql, ['id' => $this->_formID])) {
	...
	{
		if ($typeData = msSQL::sqlUnique("SELECT id, name, label, validationRules, validationErrorMsg, formType, formValues, placeholder from data_types where id= :id limit 1", ['id' => $id])) {
			return $typeData;
		} else {
	...
	{
		if ($typeData = msSQL::sqlUnique("SELECT id, name, label, validationRules, validationErrorMsg, formType, formValues, placeholder from data_types where name= :name limit 1", ['name' => $name])) {
			return $typeData;
		} else {
	...
	{
		if ($formyaml = msSQL::sqlUniqueChamp("SELECT yamlStructure from forms where id= :id limit 1", ['id' => $this->_formID])) {
	...
	{
		preg_match_all("# - ([0-9]+)#i", $formyaml, $matches);
		if (count($matches[1]) > 0) return true;
		else return false;
	}
