...
class msDicomSR extends msDicom
{
	 ...
	 * (en vue des les attacher à une donnée MedKonnect si besoin)
	...
	{
		foreach ($tab as $k => $v) {
			if (!msSQL::sqlUniqueChamp("SELECT dicomTag from dicomTags where dicomTag = :dicomTag limit 1", ['dicomTag' => $k])) {
				$data = array(
					'dicomTag' => $k,
				...
				msSQL::sqlQuery("UPDATE dicomTags set dicomCodeMeaning = :clefReadabled where dicomTag = :dicomTag ", ['clefReadabled' => $v['clefReadabled'], 'dicomTag' => $k]);
			}
		}
	}
}
