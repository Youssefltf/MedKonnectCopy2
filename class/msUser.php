...
class msUser
{
	...
	public function userIdentification()
	{
		...
		$user = msSQL::sqlUnique(
			"select id, name, CAST(AES_DECRYPT(pass,@password) AS CHAR(100)) as pass, `rank`, module, CASE WHEN secret2fa is null THEN null ELSE CAST(AES_DECRYPT(secret2fa,@password) AS CHAR(110)) END as secret2fa from people where name= :userName and lastLogFingerprint=sha1(concat(:fingerprint_partiel ,lastLogDate)) LIMIT 1",
			['userName' => $_COOKIE['userName'], 'fingerprint_partiel' => $fingerprint_partiel]
		);
		...
		if (password_verify($user['pass'], $_COOKIE['userPass'])) {
			...
			$user['prenom'] = msSQL::sqlUniqueChamp(
				"select value from objets_data where typeID= :typeIDfirstname and toID= :toID and outdated='' and deleted='' limit 1",
				['typeIDfirstname' => $name2typeID['firstname'], 'toID' => $user['id']]
			);

			$user['nom'] = msSQL::sqlUniqueChamp(
				"select value from objets_data where typeID= :typeIDlastname and toID= :toID and outdated='' and deleted='' limit 1",
				['typeIDlastname' => $name2typeID['lastname'], 'toID' => $user['id']]
			);

			if (!$user['nom']) {
				$user['nom'] = msSQL::sqlUniqueChamp(
					"select value from objets_data where typeID= :typeIDbirthname and toID= :toID and outdated='' and deleted='' limit 1",
					['typeIDbirthname' => $name2typeID['birthname'], 'toID' => $user['id']]
				);
			}
	...
	public function checkLogin($userName, $pass)
	{
		...
		$userlogin = msSQL::sqlUnique(
			"select id, name, CAST(AES_DECRYPT(pass,@password) AS CHAR(100)) as pass, CASE WHEN secret2fa is null THEN null ELSE CAST(AES_DECRYPT(secret2fa,@password) AS CHAR(110)) END as secret2fa from people where name= :userName limit 1",
			['userName' => $userName]
		);
	...
	public function set2fa()
	{
		...
		msSQL::sqlQuery(
			"UPDATE people set secret2fa=AES_ENCRYPT(:secret2fa, @password) WHERE id= :id limit 1",
			['secret2fa' => $otp->getSecret(), 'id' => $this->_userID]
		);
	...
	{
		if ($userName = msSQL::sqlUniqueChamp(
			"select name from people where id= :userID limit 1",
			['userID' => $userID]
		)) {
			return $this->checkLogin($userName, $pass);
		} else {
	...
	public static function setUserNewPassword($userID, $userPass)
	{
		...
		return msSQL::sqlQuery(
			"UPDATE people set pass=AES_ENCRYPT(:userPass, @password) WHERE id= :userID limit 1",
			['userPass' => $userPass, 'userID' => $userID]
		);
	}
	...
	public static function getUsernameFromId($id)
	{
		...
		return msSQL::sqlUniqueChamp(
			"SELECT name FROM people WHERE id= :id limit 1",
			['id' => $id]
		);
	}
	...
	public static function getUserPassByUserID($userID)
	{
		...
		return msSQL::sqlUniqueChamp(
			"select CAST(AES_DECRYPT(pass,@password) AS CHAR(100)) as pass from people where id= :userID and LENGTH(pass)>0",
			['userID' => $userID]
		);
	}
	...
	public function setUserAccountToNewPasswordRecoveryProcess()
	{
		...
		return msSQL::sqlQuery(
			"UPDATE people set lastLostPassDate=NOW(), lastLostPassRandStr = :lastLostPassRandStr  WHERE id = :userID limit 1",
			['userID' => $this->_userID, 'lastLostPassRandStr' => $this->_userPasswordRecoveryStr]
		);
	}
	...
	public function setUserAccountPasswordRecoveryProcessClosed()
	{
		if (!isset($this->_userID)) throw new Exception('UserID is not defined');
		return msSQL::sqlQuery(
			"UPDATE people set lastLostPassRandStr = NULL  WHERE id = :userID limit 1",
			['userID' => $this->_userID]
		);
	}
	...
	public static function makeRandomUniqLoginUsername($fn = '', $ln = '', $bn = '')
	{
		...
		if (msSQL::sqlUniqueChamp(
			"select name from people where name= :username limit 1",
			['username' => $username]
		)) {
			$username = self::makeRandomUniqLoginUsername($fn, $ln, $bn);
		}
