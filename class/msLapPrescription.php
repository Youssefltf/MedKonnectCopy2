...
class msLapPrescription extends msLap
{
	...
	private function _analyseAldTheriaque($ligne)
	{
		...
		$tab = [];
		if (strpos($ligne, 'SANS RAPPORT')) $rapport = 'n';
		else $rapport = 'o';
	...
	private function _getLastPrescriptionData()
	{
		...
		$marqueurs1 = [
			'lapMedicamentSpecialiteCodeTheriaque' => $name2typeID['lapMedicamentSpecialiteCodeTheriaque'],
			'toID' => $this->_toID,
			'fromID' => $this->_fromID,
			'thisSpe' => $this->_speThe
		];
		...
		if ($idLigneMedic = msSQL::sqlUniqueChamp("SELECT `instance` from `objets_data` where `typeID` = :lapMedicamentSpecialiteCodeTheriaque and `toID` = :toID and `fromID` = :fromID and `value` = :thisSpe order by `id` desc limit 1", $marqueurs1)) {
		}
		// autre patient même prat
		elseif ($idLigneMedic = msSQL::sqlUniqueChamp("SELECT `instance` from `objets_data` where `typeID` = :lapMedicamentSpecialiteCodeTheriaque and `fromID` = :fromID and `value` = :thisSpe order by `id` desc limit 1", $marqueurs2)) {
		}
		// autre prat
		else {
			$idLigneMedic = msSQL::sqlUniqueChamp("SELECT `instance` from `objets_data` where `typeID` = :lapMedicamentSpecialiteCodeTheriaque and `value` = :thisSpe order by `id` desc limit 1", $marqueurs3);
		}

		if ($idLigneMedic > 0) {

			$idLignePres = msSQL::sqlUniqueChamp("SELECT `instance` from `objets_data` where `id` = :idLigneMedic limit 1", ['idLigneMedic' => $idLigneMedic]);
			$infosMedic = json_decode(msSQL::sqlUniqueChamp("SELECT `value` from `objets_data` where `id` = :idLigneMedic  limit 1", ['idLigneMedic' => $idLigneMedic]), TRUE);
			$infosLignePres = json_decode(msSQL::sqlUniqueChamp("SELECT `value` from `objets_data` where `id` = :idLignePres  limit 1", ['idLignePres' => $idLignePres]), TRUE);
	...
	private function _dureeTotalePrescription($indexLignes = [])
	{
		...
		if ($duree['m'] == 0 and $duree['s'] < 2 and $duree['s'] > 0 and $duree['i'] == 0 and $duree['h'] == 0) {
			...
			if ($nouvelleDureeJours > 1) $nouvelleDureeJours = $nouvelleDureeJours . ' jours';
			else $nouvelleDureeJours = $nouvelleDureeJours . ' jour';
