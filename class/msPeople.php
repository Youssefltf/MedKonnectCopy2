...
class msPeople
{
	...
	public function getType()
	{
		...
		return $this->_type = msSQL::sqlUniqueChamp(
			"SELECT `type` FROM `people` WHERE `id`= :toID limit 1",
			['toID' => $this->_toID]
		);
	}
	...
	public function isExterne()
	{
		...
		return msSQL::sqlUniqueChamp(
			"SELECT `type`='externe' FROM `people` WHERE `id`= :toID limit 1",
			['toID' => $this->_toID]
		) == 1;
	}
	...
	public function getModule()
	{
		...
		return msSQL::sqlUniqueChamp(
			"SELECT module FROM people WHERE id= :toID limit 1",
			['toID' => $this->_toID]
		);
	}
	...
	public function getFromID()
	{
		...
		return msSQL::sqlUniqueChamp(
			"SELECT fromID FROM people WHERE id= :toID limit 1",
			['toID' => $this->_toID]
		);
	}
	...
	public function setPeopleExportID()
	{
		...
		$peopleExportID = msTools::getRandomStr(8, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789');

		$name2typeID = new msData();
		$name2typeID = $name2typeID->getTypeIDsFromName(['peopleExportID']);
		if (msSQL::sqlUniqueChamp(
			"SELECT pd.id from objets_data as pd where pd.typeID = :typeID and pd.deleted='' and pd.outdated='' and pd.value = :peopleExportID order by pd.id desc limit 1",
			['typeID' => $name2typeID['peopleExportID'], 'peopleExportID' => $peopleExportID]
		)) {
	...
	public function getAdministrativesDatas()
	{
		...
		if ($datas = msSQL::sql2tab(
			"SELECT d.id, d.typeID, d.value, t.name, t.label , tt.label as parentLabel, d.parentTypeID, d.creationDate
  			...
  			where d.toID = :toID and d.outdated='' and d.deleted='' and t.groupe='admin'
	...
	public function getSimpleAdminDatas()
	{
		...
		$tab = msSQL::sql2tabKey(
			"SELECT d.typeID, d.value from objets_data as d
			left join data_types as t on d.typeID=t.id
			where d.toID = :toID and d.outdated='' and d.deleted='' and t.groupe='admin'",
			"typeID",
			"value",
	...
	public function getSimpleAdminDatasByName($typesName = [])
	{
		...
		if (!empty($typesName)) {
			$sqlImplode = msSQL::sqlGetTagsForWhereIn($typesName, 'type');
			$marqueurs = array_merge($marqueurs, $sqlImplode['execute']);
			$typesSelect = " and t.name in (" . $sqlImplode['in'] . ")";
		} else {
			$typesSelect = null;
		}

		$sql = "SELECT t.name, d.value
        		from objets_data as d
        		left join data_types as t on d.typeID=t.id
			  	where d.toID= :toID and d.outdated='' and d.deleted='' and t.groupe='admin'" . $typesSelect . "
				order by t.displayOrder";

		$tab = msSQL::sql2tabKey($sql, "name", "value", $marqueurs);
	...
	{
		if ($col != ['*']) {
			if (!msSQL::sqlValidQueryCols($col, msData::COL_TABLE_DATA_TYPES)) {
        ...
        left join objets_data as od on od.typeID=dt.id and od.toID= :toID and od.outdated='' and od.deleted=''
        where dt.groupe= :groupe
        order by dt.displayOrder, dt.label", ['toID' => $this->_toID, 'groupe' => $groupe]);
	}
	...
	public static function getUsersListForService($service)
	{
		...
		if (msConfiguration::getDefaultParameterValue($service) == 'true') {
			$forbiddenIDs = msSQL::sql2tabKey("SELECT toID FROM configuration WHERE level='user' and name= :service and value='false'", 'toID', 'toID', ['service' => $service]) ?: array();
            ...
            where p.pass!='' and p.id not in ('" . implode("','", $forbiddenIDs) . "') order by identite", 'id', 'identite', $marqueurs);
		} else {
			$allowedIDs = msSQL::sql2tabKey("SELECT toID FROM configuration WHERE level='user' and name= :service and value='true'", 'toID', 'toID', ['service' => $service]) ?: array();
            ...
            left join objets_data as bn on bn.toID=p.id and bn.typeID= :birthname and bn.outdated='' and bn.deleted=''
            left join objets_data as o2 on o2.toID=p.id and o2.typeID= :firstname and o2.outdated='' and o2.deleted=''
            where p.pass!='' and p.id in ('" . implode("','", $allowedIDs) . "') order by identite", 'id', 'identite', $marqueurs);
		}
	}
	...
	public static function getUsersWithSpecificParam($param)
	{
		...
		if ($data = msSQL::sql2tab("SELECT p.id, c.value, CASE WHEN o.value != '' THEN concat(o2.value , ' ' , o.value) ELSE concat(o2.value , ' ' , bn.value) END as identite
          ...
          left join objets_data as bn on bn.toID=p.id and bn.typeID = :birthname and bn.outdated='' and bn.deleted=''
          left join objets_data as o2 on o2.toID=p.id and o2.typeID = :firstname and o2.outdated='' and o2.deleted=''
          where p.pass!='' order by identite", $marqueurs)) {
			$tab = array();
			foreach ($data as $v) {
	...
	public function getALD()
	{
		...
		$name2typeID = new msData();
		$marqueurs = $name2typeID->getTypeIDsFromName(['csAldDeclaration', 'firstname', 'lastname', 'birthname']);
		$marqueurs['toID'] = $this->_toID;

		if ($csAldID = msSQL::sql2tabKey(
			...
			where p.toID= :toID and p.typeID= :csAldDeclaration and p.deleted='' and p.outdated=''",
			'id',
			'',
			$marqueurs
		)) {
	...
	public function getAtcdStruc($parentTypeName)
	{
		...
		$marqueurs['instance'] = $marqueurs[$parentTypeName];
		$marqueurs['toID'] = $this->_toID;
		unset($marqueurs[$parentTypeName]);
		...
		if ($csAldID = msSQL::sql2tabKey(
			...
			left join objets_data as n2 on n2.toID=p.fromID and n2.typeID= :lastname and n2.outdated='' and n2.deleted=''
			left join objets_data as bn on bn.toID=p.fromID and bn.typeID= :birthname and bn.outdated='' and bn.deleted=''
			where p.toID= :toID and p.typeID= :csAtcdStrucDeclaration and p.deleted='' and p.outdated='' and p.instance= :instance ",
			'id',
			'',
			$marqueurs
		)) {
	...
	public function getAtcdAndAldCim10Codes()
	{
		...
		$msdata = new msData();
		$marqueurs = $msdata->getTypeIDsFromName(['atcdStrucCIM10', 'aldCIM10', 'csAldDeclaration', 'atcdStrucCIM10InLap']);
		$marqueurs['toID'] = $this->_toID;
		$parentsAutorises = $msdata->getTypeIDsFromName(explode(',', $p['config']['lapAtcdStrucPersoPourAnalyse']));

		if (!empty($parentsAutorises)) {
			$sqlImplode = msSQL::sqlGetTagsForWhereIn($parentsAutorises, 'parentsAuto');
			$marqueurs = array_merge($marqueurs, $sqlImplode['execute']);

			return msSQL::sql2tabSimple(
				"SELECT o.value
				from objets_data as o
				left join objets_data as p on p.id=o.instance
				left join objets_data as ac on ac.instance=o.instance and ac.typeID= :atcdStrucCIM10InLap
				where o.toID= :toID and
				((o.typeID = :atcdStrucCIM10 and p.instance in (" . $sqlImplode['in'] . ") and ac.value != 'n') or (o.typeID =  :aldCIM10 and p.typeID= :csAldDeclaration ))
				and o.deleted='' and o.outdated='' group by o.id",
				$marqueurs
			);
		} else {
			return [];
	...
	public function getAllergies($parentTypeName)
	{
		...
		$sql = "SELECT p.*, CASE WHEN n2.value != '' THEN concat(n1.value, ' ',n2.value) ELSE concat(n1.value, ' ', bn.value) END as fromName, p1.value as libelle
		from objets_data as p
		left join objets_data as p1 on p1.instance=p.id and p1.typeID = :allergieLibelleTheriaque and p1.outdated='' and p1.deleted=''
		left join objets_data as n1 on n1.toID=p.fromID and n1.typeID = :firstname and n1.outdated='' and n1.deleted=''
		left join objets_data as n2 on n2.toID=p.fromID and n2.typeID = :lastname and n2.outdated='' and n2.deleted=''
		left join objets_data as bn on bn.toID=p.fromID and bn.typeID = :birthname and bn.outdated='' and bn.deleted=''
		where p.toID= :toID and p.typeID= :allergieCodeTheriaque  and p.deleted='' and p.outdated='' and p.instance= :instance ";

		$rd['allergiesData'] = msSQL::sql2tabKey($sql, 'id', '', $marqueurs);
	...
	public function getAllergiesCodes($parentTypeName)
	{
		...
		$marqueurs['instance'] = $name2typeID[$parentTypeName];
		$marqueurs['toID'] = $this->_toID;

		$rd = [];
		$rd = msSQL::sql2tabSimple(
			"SELECT p.value
			from objets_data as p
			where p.toID= :toID and p.typeID= :allergieCodeTheriaque and p.deleted='' and p.outdated='' and p.instance= :instance ",
			$marqueurs
	...
	public function getHistoriqueData($limitStart = 0, $limitNb = 0, $datesPrecisions = '', $objetIDs = [], $instance = 0, $dataGroups = [], $critDataTypeAnnexes = [])
	{
		...
		if (is_numeric($instance) and $instance > 0) {
			$marqueurs['pinstance'] = $instance;
			$whereInstance = " and p.instance = :pinstance ";
		} else {
			$whereInstance = '';
		}

		if (!empty($dataGroups)) {
			$dataGroupsIn = msSQL::sqlGetTagsForWhereIn($dataGroups, 'datagroupe');
			$marqueurs = array_merge($marqueurs, $dataGroupsIn['execute']);
			$whereDataGroups = " and t.groupe in (" . $dataGroupsIn['in'] . ") ";
		} else {
			$whereDataGroups = '';
		}

		if (isset($objetIDs) and is_array($objetIDs) and !empty($objetIDs)) {
			$objetIDsIn = msSQL::sqlGetTagsForWhereIn($objetIDs, 'objetIDs');
			$marqueurs = array_merge($marqueurs, $objetIDsIn['execute']);
			$objetIDsSql = " and p.id in (" . $objetIDsIn['in'] . ")";
		} else {
			$objetIDsSql = '';
		}
		...
		if ($p['config']['optionGeActiverLapInterne'] == 'true') {
			$lapCompSql = " or (t.groupe = 'ordo' and  t.id = :lapOrdonnance ) ";
		} else {
			unset($marqueurs['lapOrdonnance']);
		}
		if ($p['config']['optionGeActiverLapExterne'] == 'true') {
			$lapExtCompSql = " or (t.groupe = 'ordo' and  t.id = :lapExtOrdonnance ";
		} else {
			unset($marqueurs['lapExtOrdonnance']);
		}
		...
		if (!empty($critDataTypeAnnexes)) {
			$i = 1;
			foreach ($critDataTypeAnnexes as $v) {
				$marqueurs['critDataTypeAnnexes' . $i] = $v;
				$critAnLeftJoin[] = "left join objets_data as critA" . $i . " on critA" . $i . ".instance = p.id";
				$critAnWhere[] = "critA" . $i . ".value = :critDataTypeAnnexes" . $i . " ";
				$i++;
			}
		}
		...
		left join objets_data as n2 on n2.toID=p.fromID and n2.typeID= :lastname and n2.outdated='' and n2.deleted=''
		left join objets_data as bn on bn.toID=p.fromID and bn.typeID= :birthname and bn.outdated='' and bn.deleted=''
		left join objets_data as mail on mail.instance=p.id and mail.typeID= :mailPorteur
		left join objets_data as doc on doc.instance=p.id and doc.typeID= :docType
		left join objets_data as doc2 on doc2.instance=p.id and doc2.typeID= :docOrigine
		left join objets_data as img on img.instance=p.id and img.typeID = :dicomStudyID
	...
	public function getDeathAge()
	{
		...
		if (isset($this->_birthdate)) {
			...
			$birthdate = msSQL::sqlUniqueChamp(
				"select value from objets_data where toID= :toID and typeID= :typeID and deleted!='y' and outdated!='y' order by id desc limit 1",
				['toID' => $this->_toID, 'typeID' => $typeID]
			);
		}
		...
		if (isset($this->_administrativesDatas['deathdate'])) {
			...
			$deathdate = msSQL::sqlUniqueChamp(
				"select value from objets_data where toID= :toID and typeID= :typeID and deleted!='y' and outdated!='y' order by id desc limit 1",
				['toID' => $this->_toID, 'typeID' => $typeID]
			);
		}
	...
	public function getDataHistoricalValues($name, $borneInf = "1971-01-01 00:00:00", $borneSup = "NOW()")
	{
		...
		$data = new msData();
		$marqueurs = $data->getTypeIDsFromName(['firstname', 'lastname', 'birthname', $name]);
		$marqueurs['varname'] = $marqueurs[$name];
		unset($marqueurs[$name]);
		$marqueurs['toID'] = $this->_toID;
		$marqueurs['borneInf'] = $borneInf;
		$marqueurs['borneSup'] = $borneSup;

		$sql = "SELECT v.registerDate, v.value, DATE_FORMAT(v.registerDate, '%Y-%m-%d') as dateonly, DATE_FORMAT(v.registerDate,'%H:%i:%s') as timeonly, CASE WHEN n2.value != '' THEN n2.value  ELSE bn.value END as nom, n1.value as prenom
		from objets_data as v
		left join objets_data as n1 on n1.toID=v.fromID and n1.typeID = :firstname and n1.outdated ='' and n1.deleted =''
		left join objets_data as n2 on n2.toID=v.fromID and n2.typeID = :lastname and n2.outdated ='' and n2.deleted =''
		left join objets_data as bn on bn.toID=v.fromID and bn.typeID = :birthname and bn.outdated ='' and bn.deleted =''
		where v.toID = :toID and v.typeID= :varname and v.deleted ='' and v.registerDate >= :borneInf and v.registerDate <= :borneSup
		group by v.id, n1.id, n2.id, bn.id
		order by v.registerDate desc";

		return msSQL::sql2tabKey($sql, 'registerDate', '', $marqueurs);
	}
	...
	public function getDataHistoricalValuesDistinctYears($name)
	{
		...
		return msSQL::sql2tabSimple(
			"select YEAR(registerDate) as year from objets_data where toID= :toID and typeID= :varname and deleted='' ",
			['toID' => $this->_toID, 'varname' => $name2typeID[$name]]
		);
	}
}
