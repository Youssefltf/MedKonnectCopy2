...
class msPeopleRelations extends msPeople
{
	...
	public function checkRelationExist()
	{
		...
		if (msSQL::sqlUniqueChamp("SELECT id from objets_data where toID = :toID and typeID = :typeID and value = :value and outdated='' and deleted='' limit 1", $marqueurs)) {
			return true;
		} else {
			return false;
		}
	...
	public function checkMaxGroupeRestriction()
	{
		...
		$nb = msSQL::sqlUniqueChamp("SELECT count(id) from objets_data where toID = :pratID and typeID= :typeID and outdated='' and deleted='' ", $marqueurs);
	...
	public function setRelationDeleted()
	{
		...
		// sup relation
		if ($id = msSQL::sqlUniqueChamp("SELECT id from objets_data where typeID = :typeID and toID = :toID and value = :withID and deleted='' limit 1", $marqueurs)) {
			$obj = new msObjet;
		...
		if ($id = msSQL::sqlUniqueChamp("SELECT id from objets_data where typeID= :typeID and toID = :withID and value = :toID and deleted='' limit 1", $marqueurs)) {
			$obj = new msObjet;
			$obj->setFromID($this->_fromID);
			$obj->setObjetID($id);
			$obj->setDeletedObjetAndSons();
	...
	public function getRelations($dataComp = [], $dataCompNotEmpty = [], $relationValues = [])
	{
		...
		if (!empty($this->_returnedPeopleTypes)) {
			$strictTableImplode = msSQL::sqlGetTagsForWhereIn($this->_returnedPeopleTypes, 'peopletype');
			$strictTable = " inner join people as p on p.id = o.value and p.type in (" . $strictTableImplode['in'] . ")";
			$marqueurs = array_merge($marqueurs, $strictTableImplode['execute']);
		} else {
			$strictTable = '';
		}
		...
		if (!empty($relationValues)) {
			$relationValues = msSQL::sqlGetTagsForWhereIn($relationValues, 'relval');
			$whereRelationValues = " and c.value in (" . $relationValues['in'] . ") ";
			$marqueurs = array_merge($marqueurs, $relationValues['execute']);
		} else {
			$whereRelationValues = " ";
		}
		...
		$marqueurs['userRank'] = $p['user']['rank'];
		$marqueurs['userID'] = $p['user']['id'];
		$marqueurs['relationType'] = $name2typeID[$this->_relationType];
		$marqueurs['toID'] = $this->_toID;
		$marqueurs['relationID'] = $name2typeID['relationID'];


		$sql = "SELECT o.value as peopleID, o.fromID as createBy, u2.value as currentUserStatus, c.value as typeRelation, :userRank as currentUserRank
        " . implode(" ", $champsSql) . "
        from objets_data as o
        " . $strictTable . "
        inner join objets_data as c on c.instance=o.id and c.typeID = :relationType

        left join objets_data as u on u.value=o.value and u.toID = :userID and u.typeID= :relationID and u.deleted='' and u.outdated=''
        left join objets_data as u2 on u2.instance=u.id and u2.typeID = :relationType and u2.deleted='' and u2.outdated=''

        " . implode(" ", $tablesSql) . "
        where o.toID= :toID and o.typeID= :relationID and o.deleted='' and o.outdated='' " . implode("", $notEmpty) . $whereRelationValues . "
        group by o.value, c.id, u.id, u2.id " . implode("", $groupBy) . ' ' . $orderBy;

		if ($relations =  msSQL::sql2tab($sql, $marqueurs)) {
			$typeID = $data->getTypeIDFromName($this->_relationType);
			$options = $data->getSelectOptionValue(array($typeID))[$typeID];
	...
	public function getPeopleRelationType()
	{
		...
		$marqueurs = [
			'relationID' => $name2typeID['relationID'],
			'withID' => $this->_withID,
			'toID' => $this->_toID
		];
		...
		where o.toID = :toID and o.typeID = :relationID and o.deleted='' and o.outdated='' and o.value = :withID limit 1", $marqueurs);
		if (!$typeRelation) {
			return null;
		} else {
			return $typeRelation;
	...
	public function getPeopleRelationData()
	{
		...
		$data = new msData();
		$marqueurs = $data->getTypeIDsFromName(['relationID', 'lastname', 'birthname', 'firstname']);
		$marqueurs['toID'] = $this->_toID;
		$marqueurs['withID'] = $this->_withID;

		$sql = "SELECT c.*, CASE WHEN ln.value != '' THEN concat(fn.value , ' ' , ln.value) ELSE concat(fn.value , ' ' , bn.value) END as fromIdentite
		from objets_data as o
		inner join objets_data as c on c.instance=o.id
  		left join objets_data as ln on ln.toID=c.fromID and ln.typeID = :lastname and ln.outdated='' and ln.deleted=''
		left join objets_data as bn on bn.toID=c.fromID and bn.typeID = :birthname and bn.outdated='' and bn.deleted=''
		left join objets_data as fn on fn.toID=c.fromID and fn.typeID = :firstname and fn.outdated='' and fn.deleted=''
		where o.toID = :toID and o.typeID = :relationID and o.deleted='' and o.outdated='' and o.value = :withID limit 1";

		$typeRelation = msSql::sqlUnique($sql, $marqueurs);
		if (!$typeRelation) {
			return null;
		} else {
			return $typeRelation;
