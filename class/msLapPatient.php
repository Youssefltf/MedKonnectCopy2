...
class msLapPatient extends msLap
{
	...
	public function getPatientObjetTheriaque()
	{
		...
		if (!empty($etatpatho_cim)) $tab['etatpatho_cim'] = implode(", ", $etatpatho_cim);
		else $tab['etatpatho_cim'] = '';
		...
		if (!empty(trim($p['config']['lapAllergiesStrucPersoPourAnalyse']))) {
			...
			if (!empty($hypersensibilite)) $tab['hypersensibilite'] = implode(', ', $hypersensibilite);
			else $tab['hypersensibilite'] = '';
		}
	...
	private function _checkGrossesse()
	{
		...
		if ($this->_patientAdminData['administrativeGenderCode'] != 'F') {
			...
			where pd.toID = :toID and pd.typeID = :nouvelleGrossesse and pd.outdated='' and pd.deleted='' order by pd.creationDate desc
			limit 1", $marqueurs)) {
				if (!$findGro['idFin']) {
					$objet = new msObjet;
					$objet->setToID($this->_toID);

					$moduleClass = "msModBaseCalcMed";
