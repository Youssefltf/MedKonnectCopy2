...
if (!msUser::checkUserIsAdmin()) {
	...
	if (isset($match['params']['agenda']) and is_numeric($match['params']['agenda'])) {
		...
		$whereLA[] = "a.userid= :agendaID ";
		$whereLU[] = "l.userid= :agendaID ";
		$marqueurs['agendaID'] = $p['page']['agendaID'];
	}
	if (isset($match['params']['event']) and is_numeric($match['params']['event'])) {
		$p['page']['eventID'] = $match['params']['event'];
		$whereLA[] = "a.id = :eventID ";
		$whereLU[] = "l.eventID = :eventID ";
		$marqueurs['eventID'] = $p['page']['eventID'];
	}
	...
	$marqueurs = array_merge($marqueurs, $name2typeID->getTypeIDsFromName(['firstname', 'lastname', 'birthname']));
    ...
    from agenda as a
    left join objets_data as bn on bn.toID=a.patientid and bn.typeID= :birthname and bn.deleted = '' and bn.outdated = ''
    left join objets_data as n on n.toID=a.patientid and n.typeID= :lastname and n.deleted = '' and n.outdated = ''
    left join objets_data as p on p.toID=a.patientid and p.typeID= :firstname and p.deleted = '' and p.outdated = ''
    left join objets_data as bn1 on bn1.toID=a.fromID and bn1.typeID= :birthname and bn1.deleted = '' and bn1.outdated = ''
    left join objets_data as n1 on n1.toID=a.fromID and n1.typeID= :lastname and n1.deleted = '' and n1.outdated = ''
    left join objets_data as p1 on p1.toID=a.fromID and p1.typeID= :firstname and p1.deleted = '' and p1.outdated = ''
    where " . implode(' and ', $whereLA) . "
    group by a.id, n.value, p.value, n1.value, p1.value, bn.value, bn1.value
    order by a.id desc
    limit 2000", $marqueurs)) {
    ...
    left join objets_data as n on n.toID=a.patientid and n.typeID= :lastname and n.deleted = '' and n.outdated = ''
    left join objets_data as p on p.toID=a.patientid and p.typeID= :firstname and p.deleted = '' and p.outdated = ''
    left join objets_data as bn1 on bn1.toID=l.fromID and bn1.typeID= :birthname and bn1.deleted = '' and bn1.outdated = ''
    left join objets_data as n1 on n1.toID=l.fromID and n1.typeID= :lastname and n1.deleted = '' and n1.outdated = ''
    left join objets_data as p1 on p1.toID=l.fromID and p1.typeID= :firstname and p1.deleted = '' and p1.outdated = ''
