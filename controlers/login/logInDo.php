...
if ($validation === false) {
	...
	if ($validation != false) {
		...
		if (
			'admin' == msSQL::sqlUniqueChamp("SELECT `rank` FROM people WHERE name= :p_username limit 1", ['p_username' => $_POST['p_username']]) and
		...
		syslog(LOG_WARNING, "MedKonnectEHR - echec de connexion depuis {$_SERVER['REMOTE_ADDR']}");
		closelog();
