...
/**
 * Config > ajax : créer une catégorie pour le modèle de données
 *
...
if (isset($_POST['id'])) {
	...
	$gump->validation_rules(array(
		'name' => 'required|alpha_numeric|presence_bdd,data_cat',
		'label'     => 'required',
	));
}
...
if ($validated_data === false) {
	...
	if (msSQL::sqlInsert('data_cat', $validated_data) > 0) {
		$return['status'] = 'ok';
	} else {
		$return['status'] = 'failed';
