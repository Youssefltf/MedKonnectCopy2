		...
		if (preg_match("#^([0-9]+)_[0-9]+_([0-9]+)$#", $k, $m)) {
			...
			if ($postObjetId > 0) {
				if ($v == '' and $modeAction == 'edition') {
					...
					msSQL::sqlQuery("DELETE from objets_data where instance = :postObjetId and typeID = :ordoLigneOrdoALDouPas ", ['postObjetId' => $postObjetId, 'ordoLigneOrdoALDouPas' => msData::getTypeIDFromName('ordoLigneOrdoALDouPas')]);
				}
			}
			if (isset($_POST[$k . 'CB'])) {
