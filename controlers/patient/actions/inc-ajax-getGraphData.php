...
$data = new msData;
$marqueurs = $data->getTypeIDsFromName(['poids', 'taillePatient']);
$marqueurs['patientID'] = $_POST['patientID'];

$dataBrutes = msSQL::sql2tab("SELECT dt.name, od.value, od.registerDate AS date
  FROM objets_data AS od
  LEFT JOIN data_types AS dt ON od.typeID=dt.id AND od.toID = :patientID AND deleted=''
  WHERE dt.groupe='medical' AND od.typeID in ( :poids , :taillePatient ) AND od.value != ''
  ORDER BY od.registerDate ASC", $marqueurs);
	...
	if (!array_key_exists('poids', $d)) {
		if (!array_key_exists('taillePatient', $d)) {
			if (!isset($mesureAnt) or !array_key_exists('poids', $mesureAnt) or !array_key_exists('taille', $mesureAnt)) {
				...
				$data[$k] = array(
					'poids' => array('value' => $mesureAnt['poids'], 'reel' => false),
		...
		if (!array_key_exists('taillePatient', $d)) {
			if (!isset($mesureAnt) or !array_key_exists('taille', $mesureAnt)) {
				$data[$k] = array(
					'poids' => array('value' => $d['poids'], 'reel' => true),
