...
if (!isset($delegate)) {
	if (!isset($_POST['objetID']) || $_POST['objetID'] === '') {
        ...
        WHERE od.id = :objetID limit 1", ['objetID' => $_POST['objetID']]);
		$ordoForm = $res[0]['form'];
...
if ($tabTypes = msSQL::sql2tab("SELECT p.id, p.label as optionmenu , c.label as catLabel
  ...
  where p.toID in ( :in ) and c.type='nonlap'
  group by p.id
  order by c.displayOrder, p.id in (1,2) desc, c.label asc, p.label asc", ['in' => '0, ' . $userID])) {
...
if (is_numeric($_POST['objetID'])) {
	...
	$marqueurs['objetID'] = $_POST['objetID'];
    ...
    where p.instance= :objetID and p.outdated='' and p.deleted='' and p.typeID in (:ordoTypeImpression, :ordoLigneOrdo, :ordoImpressionNbLignes)
