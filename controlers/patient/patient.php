...
if ($externe = $patient->isExterne() and ($internePatient = msSQL::sqlUniqueChamp("SELECT od.value FROM data_types AS dt LEFT JOIN objets_data AS od
  ...
  WHERE od.toID= :patientID", ['patientID' => $p['page']['patient']['id']]))) {
...
if ($externe and !$internePatient) {
	...
	$marqueurs1 = $name2typeID;
	unset($marqueurs1['personalEmail'], $marqueurs1['profesionnalEmail']);
	$marqueurs1['mobilePhoneValue'] = $data['mobilePhone']['value'];
	$marqueurs1['homePhoneValue'] = $data['homePhone']['value'];
	...
	$candidats['phone'] = msSQL::sql2tabSimple("SELECT od.toID FROM objets_data AS od left join people AS p
		...
		WHERE
		(
			(od.typeID IN (:mobilePhone, :homePhone, :telPro) AND od.value LIKE :mobilePhoneValue )
			OR
			(od.typeID IN (:mobilePhone, :homePhone, :telPro) AND od.value LIKE :homePhoneValue )
		)
		AND p.type != 'externe'", $marqueurs1);

	$marqueurs2 = $name2typeID;
	unset($marqueurs2['mobilePhone'], $marqueurs2['homePhone'], $marqueurs2['telPro']);
	$marqueurs2['personalEmailValue'] = $data['personalEmail']['value'];

	$candidats['email'] = msSQL::sql2tabSimple("SELECT od.toID FROM objets_data AS od left join people AS p
		ON od.toID=p.id AND od.outdated='' AND od.deleted=''
		WHERE typeID IN( :personalEmail, :profesionnalEmail ) and value = :personalEmailValue AND p.type!='externe'", $marqueurs2);
